...
require(dplyr)
merge2<- full_join(usonlytpast_met_agg, z_case_code)
mergestate<-full_join(usonlytpast_agg_state, z_case_state)
...
write.csv(merge2, file = "data/TP_region_past_date.csv", row.names = FALSE)
write.csv(mergestate, file = "data/TP_state_past_date.csv", row.names = FALSE)
